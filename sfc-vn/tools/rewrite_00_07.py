#!/usr/bin/env python3
"""Rewrite novel chapters 00-07 in place. Run from repo root."""
...
# Chapters 01-07 written in separate module to keep script maintainable
from rewrite_00_07_bodies import CH01, CH02, CH03, CH04, CH05, CH06, CH07

TARGETS = {
    "00-序.md": (CH00, 7000),
    "01-发送失败.md": (CH01, 20800),
    "02-空位.md": (CH02, 20800),
    "03-赵宜家.md": (CH03, 20800),
    "04-胡同.md": (CH04, 20800),
    "05-第三夜.md": (CH05, 20800),
    "06-胡同深.md": (CH06, 20800),
    "07-便利店.md": (CH07, 20800),
}


def main() -> None:
    for name, (body, target) in TARGETS.items():
        path = NOVEL / name
        path.write_text(body.strip() + "\n", encoding="utf-8")
        n = len(body.strip())
        flag = "OK" if abs(n - target) <= target * 0.08 else "WARN"
        print(f"{name}\t{n}\t(target {target})\t{flag}")
