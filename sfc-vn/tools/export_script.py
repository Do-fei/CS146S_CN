...
def build() -> str:
    ...
    lines: list[str] = [
        f"# 《{TITLE}》完整剧本",
        "",
        "本文按游戏内节点顺序收录全部对白、旁白、分支选择与九个结局，与 `game/route_*.py` 现行正文一致。",
        f"全库约 {minutes:.0f} 分钟阅读量（按每分钟 260 字估算）。单条线从开场走到一个结局，是完整的一次游玩。",
        "",
        ...
        "```",
        "",
        "三条线互不交叉。选路之后，只能走到该线的三个结局之一。另外两条路会在窗外或水箱里「看着」，但不汇合。",
        "",
        "---",
        "",
        "## 第一幕　开场",
    ...
    lines += [
        ...
        "| 倒带馆 | 交给阿青 | 馆藏 | 未完成被保管，谁也不前进。 |",
        "| 倒带馆 | 按下停止 | 漏句 | 她从停止键的缝里爬回人间。 |",
        "| 海洋馆 | 蓝色排空 | 随河 | 水位退回规矩，她去了护城河。 |",
        "| 海洋馆 | 红色对调 | 对调 | 她上岸，你成为夜场展品。 |",
        "| 海洋馆 | 白色锁死 | 锁死 | 两岸维持原样，结束被按住。 |",
        "| 无名地铁 | 坐到终点 | 无站名的早晨 | 她留在路线里，你用硬币换回白天。 |",
        "| 无名地铁 | 下一站下车 | 空白站 | 白天还给你，她继续坐下去。 |",
        "| 无名地铁 | 紧急制动 | 两站之间 | 故事安放在隧道中段，不再找替身。 |",
        "",
        "没有隐藏结局，也没有「正确」结局。三条线、九个收场并列。",
        "",
    ]
    return "\n".join(lines)
